...
decidiu sem ADR — e que nenhum arquivo da árvore implementa. A permissão para a aresta
do aviso, hoje, vem do
[ADR-0020](../adr/0020-o-aviso-de-conclusao-e-a-subsuncao-do-adr-0008.md#decisão), que
recorta a proibição do ADR-0008 para admiti-la. Ele não repete nenhuma aresta do
diagrama anterior.
...
flowchart TB
    ...
    ST -.->|" aviso de conclusão, HTTP,<br/>disparado e esquecido "| LP
    LP -->|" consulta de confirmação, ao receber o aviso "| ST
    LP -->|" observação "| RB
...
| `lab-plane`               | `system-under-test`                                                              | chamada de passo, por rede; sentido inverso proibido, exceto o aviso de conclusão | executar cada passo da operação medida                                                          | `decidido/não implementado` — os dois processos sobem; nenhuma chamada existe                                                                      | nenhum                        | [ADR-0008](../adr/0008-os-dois-planos-em-processos-separados.md#decisão), recortado pelo [ADR-0020](../adr/0020-o-aviso-de-conclusao-e-a-subsuncao-do-adr-0008.md#decisão); `system-under-test/pom.xml` não declara dependência do `lab-plane`                                      |
| `system-under-test`       | `lab-plane`                                                                      | callback HTTP, disparado e esquecido, fora da janela medida                       | avisar que o processo medido terminou, disparando a consulta de confirmação                     | `decidido/não implementado` — decidida sem ADR em 2026-08-14, e a tensão com o ADR-0008 resolvida pelo ADR-0020 no mesmo dia; nenhum código existe | nenhum; forma não decidida    | [feature-card.md, Regras de negócio](../features/deteccao-de-divergencia-entre-fontes/feature-card.md#regras-de-negócio); permitida pelo [ADR-0020](../adr/0020-o-aviso-de-conclusao-e-a-subsuncao-do-adr-0008.md#decisão)                                                          |
| `lab-plane`               | `system-under-test`                                                              | consulta de confirmação, disparada ao receber o aviso, fora da janela medida      | confirmar o consolidado por recurso, e detectar divergência com o stream de CDC                 | `decidido/não implementado` — sem ADR, em 2026-08-13, refinada em 2026-08-14; nenhum código nem rota                                               | nenhum; forma não decidida    | [feature-card.md, Regras de negócio](../features/deteccao-de-divergencia-entre-fontes/feature-card.md#regras-de-negócio); [card](../features/deteccao-de-divergencia-entre-fontes/feature-card.md)                                                                                  |
| `lab-plane`               | RabbitMQ                                                                         | AMQP; mensagem de negócio, sem LSN                                                | levar a abertura, a observação e a mensagem terminal — veredito ou rótulo — até o `lab-journal` | `decidido/não implementado`                                                                                                                        | nenhum                        | [ADR-0014](../adr/0014-o-broker-na-travessia-da-observacao-e-o-cursor-monotonico-do-replay.md#o-evento-sai-do-passo-pelo-broker); [example mapping](../features/deteccao-de-divergencia-entre-fontes/example-mapping.md#regras)                                                     |
| RabbitMQ                  | `lab-journal`                                                                    | AMQP; persiste, depois emite                                                      | alimentar o caderno com a abertura, a observação e a mensagem terminal da execução              | `decidido/não implementado`                                                                                                                        | nenhum                        | [ADR-0016](../adr/0016-o-streaming-e-o-replay-do-log-de-observacoes.md#no-lab-journal-a-ordem-é-serial-persiste-depois-emite); [example mapping](../features/deteccao-de-divergencia-entre-fontes/example-mapping.md#regras)                                                        |
...
| RabbitMQ                  | `lab-plane`                                                                      | AMQP; o filtro por execução acontece no consumidor                                | contar `commits`, ler `final_value`, somar `Σ amount`                                           | `decidido/não implementado` — exige `lab-plane` em réplica única                                                                                   | nenhum                        | [ADR-0012](../adr/0012-o-broker-no-caminho-do-veredito-e-a-dispensa-que-ele-exigiu.md#decisão), que exige a réplica única no mesmo parágrafo do filtro; a soma do predicado vem do [ADR-0013](../adr/0013-a-proveniencia-da-fonte-como-criterio-da-proibicao-do-oraculo.md#decisão) |
...
1. **O contador do oráculo exato está decidido, e não implementado.** `commits`,
   `initial_value` e `final_value` vêm do WAL por replicação lógica, e nunca de um
...
matriz acima. **O caminho pelo qual o veredito, ou o rótulo do instrumento, chega ao
frontend saiu desta lista em 2026-08-14**: ele viaja pela mensagem terminal que o
`lab-plane` já publica no RabbitMQ, o `lab-journal` persiste e emite por SSE, sem
`Backend For Frontend` e sem aresta nova — nenhuma linha desta matriz muda por isso.
