...
- **Alterado por:**
  ...
  regra de que `initial_value` e `final_value` são "lidos do banco"
  ...
  deixam de valer.
  `initial_value` passa a vir do `INSERT` do estado inicial, e `final_value` do último
...
```
lost_operations = commits − (final_value − initial_value)
```

`commits` é o número de passagens pela fronteira `AFTER_COMMIT`, contadas por tentativa.
`initial_value` e `final_value` são lidos do banco, antes de o primeiro worker começar e
...
execuções.** O oráculo lê `initial_value`; ele não o cria. A restrição de que o
...
produza `commits` diferente de `final_value − initial_value` por causa do banco, e não
