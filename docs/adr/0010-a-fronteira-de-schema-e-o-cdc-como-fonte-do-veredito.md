...
commits − (final_value − initial_value)`, lidos do banco antes do primeiro worker e
depois do último
...
por replicação lógica.** `initial_value` passa a ser o valor do `INSERT` que cria o
estado inicial da execução. `final_value` passa a ser o último valor de
...
- `initial_value` e `final_value` passam a vir da mesma fonte: o estado inicial é
...
- **Se a proibição alcança também a leitura direta não está decidido. `Pergunta em
  aberto`
  ([`E-37`](../fila-de-decisoes.md#e-37--o-que-a-proibição-de-derivar-estado-de-stream-alcança)).**
  `initial_value` vem do `INSERT` do estado inicial; `final_value`, do último evento do
  stream — nenhum dos dois soma, diferente da reconstrução que o predicado exige. `O20`
  já trata a leitura direta de `initial_value` como aceitável, sem declarar se ela
...
| 2026-08-14 | `## Contexto`, `## Decisão` e `## Consequências` | os dois identificadores do oráculo exato deixam de ser tradução palavra a palavra e passam a ter a ordem do inglês, sem que nenhum número, relação ou argumento mude: `value_initial` vira `initial_value` e `value_final` vira `final_value`                                                                                                                                         | decidido pela pessoa em 2026-08-14, logo depois do patch do mesmo dia que grafou os identificadores em inglês: `value_initial` e `value_final` preservavam a ordem portuguesa, em que o adjetivo segue o substantivo, e por isso não eram inglês. A grafia antiga sobrevive em `adr/arquivo/`, que nunca é editado, e por isso a uniformidade não é alcançável. **A alteração excede o limite ordinário do patch**, que NÃO DEVE alcançar `## Decisão`, a justificativa, a alternativa descartada nem a consequência — ela foi autorizada explicitamente, e fica registrada aqui em vez de ficar sem rastro |
