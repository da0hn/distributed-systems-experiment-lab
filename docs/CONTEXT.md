...
**Quatro identificadores deixaram de seguir o parágrafo acima em 2026-08-14, por decisão
explícita da pessoa.** `perdidas`, `sucessos`, `value_inicial` e `value_final` foram
grafados em inglês no corpus inteiro — ADRs aceitos inclusive, cada um com a linha dele em
`## Patches aplicados` —, e passaram a ser `lost_operations`, `successes`, `initial_value`
e `final_value`. Os dois últimos passaram antes por `value_initial` e `value_final`, e a
correção veio no mesmo dia: aquela forma preservava a ordem portuguesa, em que o adjetivo
segue o substantivo, e por isso era tradução palavra a palavra, e não inglês. As linhas de
de/para deles continuam na tabela, e não são resíduo: a grafia portuguesa sobrevive em
`adr/arquivo/`, que nunca é editado, e "atualizações perdidas" e "operações perdidas"
seguem sendo prosa, e não identificador. O parágrafo acima continua descrevendo todo o
resto do corpus.
...
e o dono da forma é [`schemas/sut.md`](architecture/schemas/sut.md#o-schema-do-sistema-medido-sut).
...
O oráculo do contador, que produz `lost_operations = commits − (final_value − initial_value)`.
...
`commits` iguala `final_value − initial_value`. A estratégia é `ATOMIC_UPDATE`.
