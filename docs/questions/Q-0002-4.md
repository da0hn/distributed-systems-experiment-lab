...
O oráculo do ADR-0002 lê `value_initial` antes do primeiro worker. Aquele ADR não diz
...
`value_initial` e `value_final` venham da mesma fonte. A distinção só passou a existir
depois que o
...
ainda diz que o oráculo "lê `value_initial` antes do primeiro worker". Fechar uma das
