...
O oráculo do ADR-0002 lê `initial_value` antes do primeiro worker. Aquele ADR não diz
...
[`schemas/sut.md`](../architecture/schemas/sut.md#o-que-o-diagrama-do-sut-não-desenha), e não
...
**Nota, 2026-08-12.** A linha
[`E-86`](../fila-de-decisoes.md#e-86--a-regra-r18-do-e1-é-viva-e-a-única-evidência-dela-está-no-arquivo-congelado)
foi aberta ao lado desta questão, e não a substitui. Esta pergunta é de **ciclo de
vida** — quem escreve o estado inicial, quem limpa e quando. Aquela é de
**proveniência** — se a escrita precisa ser observável no stream, para que
`initial_value` e `final_value` venham da mesma fonte. A distinção só passou a existir
depois que o
[ADR-0010](../adr/0010-a-fronteira-de-schema-e-o-cdc-como-fonte-do-veredito.md#decisão)
tirou o `SELECT` do caminho do veredito; o parágrafo de abertura, escrito antes dele,
ainda diz que o oráculo "lê `initial_value` antes do primeiro worker". Fechar uma das
duas **não** fecha a outra.
