...
| O que impede um veredito malformado               | o banco, no `INSERT`: `CHECK`, `NOT NULL` e tipo de coluna          | nada no banco; quem lê valida contra a versão de forma declarada               | nada no banco: `value numeric` aceita 7 onde só 0 e 1 significam algo |
...
| Correção de um resultado publicado                | linha nova de `experiment`, e a rodada congela a versão             | entrada nova que sucede a anterior; a superada permanece                       | não tratada: `experiment` é imutável depois da primeira execução      |
| `UPDATE` no desenho                               | existe, num trigger só, sobre `experiment.updated_at`               | não existe, e a garantia vem de `REVOKE`, não do esquema                       | existe, no mesmo trigger de `experiment.updated_at`                   |
| Cursor do replay                                  | coluna própria em `observation`, contígua por execução              | é o número de sequência do livro; nada foi acrescentado para o stream          | coluna própria em `observation`, contígua por execução                |
| Evento terminal do stream                         | derivado de `run.final_cursor`; não é linha do log                  | é o selo, última entrada do livro                                              | é linha de `observation`, e `final_cursor` a espelha                  |
| Relógio do banco                                  | `DEFAULT now()` em `experiment`, pela carve-out de metadado de CRUD | nenhum `DEFAULT`: o digest cobre o instante, e `now()` quebraria a cadeia      | `DEFAULT now()` em `experiment`, pela mesma carve-out                 |
...
**Caro: o esquema para de distinguir um booleano de uma contagem.** `value numeric` aceita
