# CLAUDE.md — trabalhando dentro de `docs/`

Instruções operacionais para editar esta pasta. O contexto do projeto está no
[`CLAUDE.md` da raiz](../CLAUDE.md); o mapa da pasta está em [`README.md`](README.md).
...
As convenções gerais estão no [`CLAUDE.md` da raiz](../CLAUDE.md), seção `## Convenções de
