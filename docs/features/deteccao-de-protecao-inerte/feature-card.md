...
| R8  | O oráculo **DEVE** conferir a contiguidade da sequência de LSN antes de somar. Um buraco **DEVE** invalidar a execução, e o oráculo **NÃO DEVE** produzir veredito nesse caso.            | [ADR-0013, Decisão](../../adr/0013-a-proveniencia-da-fonte-como-criterio-da-proibicao-do-oraculo.md#decisão)                                                                                                       | pendente     |
...
pede **recência**, somar pede **completude** — e é isso que `R8` repõe.
...
R1 a R8 verificadas por teste. **R8 pela injeção**: retirado um evento de `INSERT` do
stream, a execução **DEVE** terminar invalidada e **NÃO DEVE** produzir veredito verde. O
