...
`Aceito`, e de três decisões absorvidas aqui: a distinção entre higiene e invalidação, a
tabela de execuções ativas no schema `lab_plane`, e os três caminhos de saída dessa
lista, origem de `R7`.
...
Os três caminhos que R7 fixa produzem o fluxo a seguir. A sentinela passa a remover a
linha porque o limite de espera existe justamente para o caso em que nenhuma marca
chega: lê-la como caminho que não remove faria toda execução bem-sucedida esperar o
limite estourar.
...
| R5    | Duas réplicas do `lab-plane` sobem ao mesmo tempo, lendo a mesma tabela de execuções ativas do schema `lab_plane` (R4)                                | Hipótese não decidida, condicionada ao ramo `fila clássica` da escolha do sink: o broker distribui os eventos do backlog entre as duas réplicas, sem que nenhuma processe sozinha a sequência completa de uma execução | Nenhuma das duas sabe dizer, sozinha, qual causou um descarte — contraexemplo dependente do sink do RabbitMQ, ainda não escolhido, e não cenário                                                                                  |
...
não em memória por réplica — a alternativa descartada, porque em memória a resposta some
num reinício. O mecanismo exato pelo qual duas réplicas, lendo a mesma tabela, ainda
produzem um descarte ambíguo depende de qual sink do RabbitMQ recebe os eventos — o de
fila clássica, sobre AMQP 0-9-1, ou o de stream, sobre o protocolo de stream —, e essa
escolha continua aberta, porque ela decide qual fenômeno de saturação o grupo B consegue
reproduzir. Por isso este contraexemplo fica registrado como dependente dela (P6), e não
como um mecanismo concreto encenável hoje.
...
flowchart TD
    ...
    ADR -->|" mecanismo concreto<br/>depende de "| SINK{"fila clássica<br/>ou stream do RabbitMQ?"}
    SINK -->|" sem essa escolha "| PA["o contraexemplo fica<br/>como pergunta em aberto, P6"]
```

### Contraexemplo — o limite de espera que ignora o adaptador de relógio

R7 recusa, para o limite de espera, a exceção concedida a um limite que não entra em
veredito: aqui a assimetria de risco pesa mais que o custo do adaptador — aplicar a
regra do relógio a um limite que não precisava dela custa um adaptador, e não aplicá-la
a um que precisava quebra a reprodutibilidade em silêncio, meses depois.
...
| #  | Pergunta                                                                                                                                                                                                                                                                                                                                                                            | Origem                                                                                                                                                      |
|----|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------|
| P1 | Qual é a forma da tabela de execuções ativas — colunas, chave e migração?                                                                                                                                                                                                                                                                                                           | A escolha de guardar a lista em tabela não fixou coluna, chave nem migração.                                                                                |
| P4 | Uma execução encerrada pelo **limite de espera** produz veredito? Ela é candidata natural a um quinto valor da classificação do veredito zero, que o ADR-0004 já fixou com quatro, e acrescentar um quinto é decisão arquitetural nova.                                                                                                                                             | Os três caminhos de saída da lista foram fixados sem decidir isso.                                                                                          |
| P5 | Resolvida em parte: o `DEVE` de réplica única do `lab-plane` (ADR-0012) mudou de lugar e vira critério de aceite normativo (`replicas: 1`) na issue #2 do `homelab-infrastructure`. A segunda instância deliberada é do `system-under-test`, cujo número de réplicas nenhum ADR fixa; o que sobra é o `selfHeal` desfazer uma escala feita fora do Git, antes ou durante a medição. | [ADR-0019](../../adr/0019-a-entrega-sai-do-deploy-e-a-imagem-ganha-tag-semantica.md#a-réplica-única-do-lab-plane-passa-a-ser-critério-de-aceite-na-issue-2) |
| P6 | Qual mecanismo concreto faz duas réplicas do `lab-plane`, lendo a mesma tabela de execuções ativas, ainda produzirem descarte ambíguo? A resposta depende de qual sink do RabbitMQ for escolhido.                                                                                                                                                                                   | A escolha entre o sink de fila clássica, sobre AMQP 0-9-1, e o de stream ainda não foi feita.                                                               |
| P7 | Qual é o limite de espera, e ele é por execução ou global?                                                                                                                                                                                                                                                                                                                          | Um número escrito aqui seria decisão que ninguém tomou.                                                                                                     |
| P8 | O cancelamento explícito e o abandono por limite de espera se distinguem no registro da execução, ou os dois produzem o mesmo estado?                                                                                                                                                                                                                                               | Os dois tiram a linha da lista; se o resultado guarda por qual deles, ninguém decidiu.                                                                      |

P2 e P3 foram respondidas por `R7`: os três caminhos de saída da lista de execuções
ativas.
...
|----------------------------------------------------------------|------------------------------------------------------|
| A forma da tabela de execuções ativas                          | a decisão sobre colunas, chave e migração            |
| O valor do limite de espera, e se ele é por execução ou global | a decisão que fecha P7                               |
| Se o registro distingue cancelamento de abandono               | a decisão que fecha P8                               |
| Se uma execução encerrada por limite produz veredito           | a decisão que fecha P4                               |
| O mecanismo concreto do contraexemplo de `R5`                  | a escolha de qual sink do RabbitMQ recebe os eventos |

**A garantia formal de réplica única na entrega saiu desta lista.** O gatilho — a
decisão sobre onde os manifests do laboratório vivem — disparou, e a resposta está em
P5: os manifests foram para o `homelab-infrastructure` sem tratar da réplica única, e o
`DEVE` do ADR-0012 passou a ser critério de aceite na issue #2 daquele repositório.
