...
| R2    | um recurso com `capacity = 100`, três alocações somando `70`, e nenhuma alocação órfã                 | o endpoint é consultado            | ele devolve, para aquele recurso, `final_value`, `capacity = 100`, `soma = 70`, `contagem = 3`; órfãs zero |
...
tensão sem resposta foi recusado: um card não contradiz um ADR aceito, e a contradição
vista é decisão arquitetural que precisa ser tomada, e não anotada de lado. **A pessoa
