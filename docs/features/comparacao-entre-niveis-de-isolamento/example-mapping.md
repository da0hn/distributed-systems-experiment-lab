...
Companheiro de [`feature-card.md`](feature-card.md). As regras vêm do fecho de
...
| R1, contraexemplo | E5 com `NONE` — nenhuma estratégia de concorrência no código da aplicação          | a mesma execução roda sob `SERIALIZABLE`                 | a invariante não quebra sob `SERIALIZABLE` — o mesmo fato que o `Exemplo 7.4` de `deteccao-de-protecao-inerte` registra, sem concluir proteção             |
...
| #  | Pergunta                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Origem                                                                                                                                                                                                                                                 |
|----|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| P1 | A `R7` de `deteccao-de-protecao-inerte` — aprovada, e citando este mesmo fecho — já descreve a comparação sob os três níveis para o E5. O fecho de `E-87` deixa explícito que decidir entre absorver o texto dela neste card novo, ou mantê-la como ponteiro no card antigo, é trabalho da redação, e não da linha da fila. Custo de absorver: apagar ou renumerar `R7` desloca `R8` a `R11` daquele card; `R11` também descreve o braço `SERIALIZABLE`, e a renumeração quebraria a citação por nome que `R15` de `execucao-de-experimento` faz a `R8` — citação em prosa, não em âncora, que o verificador de citações não detecta. Custo de manter como ponteiro: a mesma comparação vive descrita em dois cards, e nada impede que um seja editado sem o outro. Este card não decide: `R7` permanece intocada, e `R1` a `R3` aqui citam o mesmo fecho como evidência, sem repetir o texto dela. | [`E-87`, fecho](../../adr/fila-de-decisoes.md#e-87-fecha-em-card-novo-para-a-comparação-entre-níveis-de-isolamento-escolhida-em-2026-08-12) e [`deteccao-de-protecao-inerte`, R7](../deteccao-de-protecao-inerte/feature-card.md#regras-de-negócio)    |
| P2 | Onde o nível de isolamento é declarado — no experimento, na conexão, ou em outro lugar? O `ADR-0002` deixou isso fora de escopo por escrito, e nenhum ADR posterior o fixou.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | [`ADR-0002`, O que este ADR não decide](../../adr/0002-o-dominio-minimo-e-os-dois-oraculos.md#o-que-este-adr-não-decide) e [`deteccao-de-protecao-inerte`, Example Mapping, P1](../deteccao-de-protecao-inerte/example-mapping.md#perguntas-em-aberto) |
| P3 | O retry exigido depois do SQLSTATE `40001` é da estratégia de concorrência ou do runtime? A resposta muda o que compõe "o custo" de um nível que protege.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | [`deteccao-de-protecao-inerte`, Example Mapping, P2](../deteccao-de-protecao-inerte/example-mapping.md#perguntas-em-aberto)                                                                                                                            |
...
| A relação entre "protegido" desta capacidade e o `protegido` do `ADR-0004` | a decisão que resolver `P6`, ajustando a classificação do zero para o eixo do nível de isolamento, ou explicando por que não precisa |

## O que não virou cenário, e por quê

Nenhuma regra deste card tem `Aprovada por` preenchido — todas nasceram `pendente`
nesta redação, pela decisão do fecho de `E-87` de tratar a comparação como capacidade
própria, ainda sem aprovação de pessoa sobre o texto exato. Regra `pendente` NÃO DEVE
virar cenário Gherkin, e por isso este ciclo não produz `behavior.feature` para este
card.
