...
Cobre o eixo do nível de isolamento como parâmetro de execução, hoje exercido pelo
experimento **E5** (`write-skew-inert-protection`).
...
resultado mais desconfortável do laboratório — `OPTIMISTIC` sob `READ COMMITTED` quebra
a invariante do E5 sem lançar exceção, porque inserir uma alocação não incrementa a
versão de linha alguma.

Resultado esperado: o mesmo experimento roda sob os três níveis; o relatório diz qual
protege e a que custo; nenhuma combinação é recusada; e o par declarado aparece ao lado
de cada veredito.
...
- **O oráculo do experimento subjacente** — hoje, o do E5 — avalia cada execução, pelo
  mecanismo de
  [`deteccao-de-protecao-inerte`](../deteccao-de-protecao-inerte/feature-card.md#atores-e-gatilho).

Gatilho: uma execução de experimento declarada para comparação entre níveis.
...
- O eixo do nível de isolamento, ortogonal ao eixo da estratégia de concorrência.
- A execução do mesmo experimento sob os três níveis do PostgreSQL.
...
- O oráculo exato do contador (E1/E3) e o oráculo do predicado (E5) — cada um com card
  próprio:
...
Como as três execuções de uma mesma comparação se agrupam num relatório único é
`Pergunta em aberto` — nenhum contrato formaliza isso hoje, e `Q-INT-1` já cobre a
ausência geral de contrato do relatório, em
[`integrations.md`](../../architecture/integrations.md#perguntas-em-aberto).

## Riscos e decisões pendentes

- **Onde o nível é declarado.** Herdado de `deteccao-de-protecao-inerte`; o `ADR-0002`
...
- **Generalização além do E5.** Nenhum documento aplica isto ao oráculo exato do E1/E3.
- **A métrica de "custo".** A taxa de aborto de `execucao-de-experimento` é candidata,
  sem confirmação.
- **Se "protege" é o `protegido` do `ADR-0004`.** `P6`, no Example Mapping.
...
R1 a R3 verificadas por teste, sobre o E5. As três execuções produzem três veredictos:
`READ COMMITTED` e `REPEATABLE READ` violam a invariante, `SERIALIZABLE` a preserva com
SQLSTATE `40001`. Nenhuma combinação é recusada antes de rodar. Cada linha do relatório
traz o par ao lado do veredito, e não um rótulo único que colapse os dois eixos.
...
- [`deteccao-de-protecao-inerte`](../deteccao-de-protecao-inerte/feature-card.md) — o E5,
  hoje a única instância concreta
- [`execucao-de-experimento`](../execucao-de-experimento/feature-card.md) — o relatório
  reusado
...
- [`ADR-0004`](../../adr/0004-o-estatuto-da-barreira-e-o-diagnostico-da-nao-ocorrencia.md#o-zero-é-classificado-e-a-classificação-tem-quatro-valores),
  `Aceito` — a classificação que `P6` questiona
