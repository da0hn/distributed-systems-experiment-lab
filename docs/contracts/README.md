...
0014 e do ADR-0016. **O que mudou foram os motivos.**
...
| OpenAPI — frontend → `lab-journal`                  | decidida, não formalizada | mesmo ADR: leitura e streaming saem do `lab-journal`; o transporte é SSE com `Last-Event-ID` e replay por cursor, decidido pelo [ADR-0016](../adr/0016-o-streaming-e-o-replay-do-log-de-observacoes.md#o-replay-por-cursor-é-o-único-mecanismo-com-ou-sem-histórico-completo) e já pressuposto por `frontend/nginx.conf:18-28`; nenhum evento foi especificado                                                                                                                                                                                                         | a primeira rota HTTP escrita          |
...
| AsyncAPI — `lab-plane` → RabbitMQ → `lab-journal`   | decidida, não formalizada | [ADR-0014](../adr/0014-o-broker-na-travessia-da-observacao-e-o-cursor-monotonico-do-replay.md#justificativa) pôs o mesmo broker no caminho da observação, e escreveu a dispensa que esse uso exigiu; a contrapressão segue sem decisão, nas [negativas do mesmo ADR](../adr/0014-o-broker-na-travessia-da-observacao-e-o-cursor-monotonico-do-replay.md#negativas), e a forma concreta do registro persistido é pergunta em aberto do [ADR-0016](../adr/0016-o-streaming-e-o-replay-do-log-de-observacoes.md#negativas), que é quem decide o que o `lab-journal` grava | o primeiro evento publicado no broker |
...
| manifests de entrega                        | [ADR-0019](../adr/0019-a-entrega-sai-do-deploy-e-a-imagem-ganha-tag-semantica.md#os-manifests-vivem-no-homelab-infrastructure-e-deploy-não-nasce-aqui)                                                        | Kustomize, **`deploy/` não nasce aqui, por decisão** |
